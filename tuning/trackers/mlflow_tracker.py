...
class RunIDExporterMlFlowCallback(MLflowCallback):
    """
    Custom MlflowCallBack callback is used to export run id from Mlflow
    ...
    def setup(self, args, state, model):
        ...
        active_run = self._ml_flow.active_run()
        if not active_run:
            return
        ...
        export_path = os.path.join(
            self.run_id_export_path, MLFLOW_RUN_URI_EXPORT_FILENAME
        )
        with open(export_path, "w", encoding="utf-8") as f:
            f.write(json.dumps({"run_name": run_name, "run_uri": run_uri}))
            self.logger.info("Mlflow tracker run uri id dumped to " + export_path)
...
class MLflowTracker(Tracker):
    ...
    def get_hf_callback(self):
        ...
        mlflow_callback = RunIDExporterMlFlowCallback()
