...
dataloader:
    type: hf/stateful
    streaming: true
    data_handler_routines_path: "run.py"
datasets:
  - name: dataset1
    sampling:
      ratio: 0.3
...
To this end, one way to design is we can take a parameter in the `Dataloader` like `data_handler_routines_path`
which represents a user provided python code which contains data handler implementation and a map representing
KV pairs of `names:data_handlers` which the `Dataloader` can register using its `register_data_handler` routine.
...
The HF dataloader also implements functionality to process different type of files, if not supported automatically via
`load_dataset`, we need to implement loading code for different type of data formats like this.
...
def load_files_by_type(data_files, kwargs=None):
    ...
    if e == "json" or e == "jsonl":
        return HFJSONDatasetLoader(data_files, **kwargs)
    elif e == "parquet":
        return HFParquetDatasetLoader(data_files)
    elif e == "arrow":
        return HFArrowDatasetLoader(data_files)
```

The individual handlers are implemented like this for example, this is a repurpose of the code `tuning/utils/preprocessing_utils.py::load_hf_dataset_from_file`

```
class HFJSONDatasetLoader:
        ...
        for f in files:
            with open(f, "r", encoding="utf-8") as json_file:
                ...
                if file_extension == ".jsonl":
                    ...
                    raise ValueError(f"HFJSONDatasetLoader does not support {file_extension} format")
